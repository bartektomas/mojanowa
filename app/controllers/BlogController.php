<?php
  ...
  class BlogController extends BaseController {
    ...
    public function changeProfilePicture($name)
    {
      ...
      {
        Input::file('picture')->move('uploads/pictures', Input::file('picture')->getClientOriginalName());
        $config['picture'] = 'uploads/pictures/'.Input::file('picture')->getClientOriginalName();
        $blog->config = json_encode($config);
        $blog->save();
        return Redirect::route('blogsettings', array('name' => $name))->withSuccess('Settings Updated');
      }
    ...
    public function publicBlog($name)
    {
      $blog = Blog::where('name', '=', $name)->first();
      ...
      return View::make('theme')->withPosts($posts)->withBlog($currentBlog);
    }
    ...
    public function processCustomiseBlog($name)
    {
      ...
      $config['customcss'] = substr(Input::get('customcss'),0,60000);
