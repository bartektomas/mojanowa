<?php
  ...
  class PostController extends BaseController {
    ...
    public function saveText()
    {
      ...
      {
        if(strlen(htmlentities($_POST['content'])) > 65000)
        {
          echo "<img src='".URL::asset('assets/images/fuckoff.png')."' />";
          exit();
        }
    ...
    public function saveLink()
    {
      ...
      {
        if(strlen(htmlentities($_POST['caption'])) > 65000)
        {
          echo "<img src='".URL::asset('assets/images/fuckoff.png')."' />";
          exit();
        }
	...
	$_POST['tags'] = str_replace('#','',$_POST['tags']);
        $validator = Validator::make(
                              Input::all(),
                              array('file' => 'required|max:163840')
        );
  ...
  $_POST['tags'] = str_replace('#','',$_POST['tags']);
    ...
    public function delete($id)
    {
      ...
      //$post->rebloggedPosts()->post()->delete();

      $reblogs = $post->rebloggedPosts()->get();
      if($post->rebloggedPost)
      {
      	$post->rebloggedPost->delete();
      }
    ...
    public function update($id)
    {
      ...
      if($post->type == 'regular')
      {
        ...
        if($post->regularPost->type == 'text' || $post->regularPost->type == 'chat')
        {
          ...
          $content['title'] = htmlentities(Input::get('title'));
          $content['content'] = htmlentities(Input::get('content'));
        ...
        {
          $content = json_decode($post->regularPost->content,true);
          $content['source'] = htmlentities(Input::get('source'));
          $content['content'] = htmlentities(Input::get('content'));
          $post->regularPost->content = json_encode($content);
          $post->regularPost->save();
        }
      ...
      {
        if(strlen(htmlentities($_POST['commentt'])) > 65000)
        {
          echo "<img src='".URL::asset('assets/images/fuckoff.png')."' />";
          exit();
        }
        $content = json_decode($post->rebloggedPost->content,true);
        $content['comment'] = htmlentities(Input::get('comment'));
